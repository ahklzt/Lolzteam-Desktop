...
const linuxDesktopFileName = (scheme: string) =>
  `lzt-${scheme}-handler.desktop`;
...
const run = (cmd: string, args: string[]) =>
  new Promise<void>((done) => {
    execFile(cmd, args, (err) => {
      if (err) log.warn(`[protocol] ${cmd} failed:`, err.message);
      done();
    });
  });
...
const resolveDevProjectRoot = (): string => {
  ...
  if (argvEntry) {
    const absEntry = isAbsolute(argvEntry)
      ? argvEntry
      : resolve(process.cwd(), argvEntry);
...
const writeLinuxDesktopFile = async (scheme: string) => {
  ...
  const projectRoot = resolveDevProjectRoot();
  const productName = app.getName?.() || "Lolzteam Desktop";
  const content = buildLinuxDesktopFile(
    scheme,
    electronBinary,
    projectRoot,
    productName,
  ...
  try {
    ...
    if (needWrite) {
      ...
      log.info(`[protocol] wrote ${target}`);
    ...
    log.warn("[protocol] failed to register linux desktop file:", err);
  }
};
