...
const runCredentialLogin = async (
  ...
  const session = new LoginSession(platform, {
    machineId: true,
    ...(params.httpProxy ? { httpProxy: params.httpProxy } : {}),
    ...(params.socksProxy ? { socksProxy: params.socksProxy } : {}),
  })
  ...
  if (start.actionRequired) {
    ...
    if (has(EAuthSessionGuardType.DeviceCode) && params.sharedSecret) {
      ...
      if (!params.emailCode) return failWith({ kind: 'needs-email-code' })
      try {
  ...
  try {
    ...
    return failWith({ kind: 'unknown', message: err instanceof Error ? err.message : String(err) })
  }
