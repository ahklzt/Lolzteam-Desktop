...
export const extractLlmCookieArray = (details: AccountLoginDetails): InjectableCookie[] => {
  ...
  sources.push(secrets.login, secrets.password)
...
export const extractEmailCreds = (
  ...
  const email =
    ...
    asString(secrets.login)
  const password =
    pick(eld, 'password') ??
    pick(ld, 'password') ??
    asString(secrets.account_password) ??
    asString(secrets.password)
  return email && password ? { email, password } : null
}

export const emailPasswordFor = (details: AccountLoginDetails): string | null => {
  const creds = extractEmailCreds(details)
  return creds ? `${creds.email}:${creds.password}` : null
}

export const extractLlmKey = (details: AccountLoginDetails): string | null => {
  const secrets = details.item as Record<string, unknown>
  log.debug(
